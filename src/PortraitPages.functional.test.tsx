...
describe("Portrait pages", () => {
  ...
  it("connects the real booster purchase and owned pack actions", async () => {
    ...
    render(
      <PortraitShop
        ...
        onBuy={buy}
        onOpen={open}
        monetization={loadMonetization()}
      />,
    );
    expect(
      screen.getByRole("button", { name: /Guérisseurs Émeraude/ }),
    ).toBeInTheDocument();
    await userEvent.click(
      screen.getByRole("button", { name: /Guérisseurs Émeraude/ }),
    );
    expect(
      screen.getByRole("heading", { name: "Guérisseurs Émeraude" }),
    ).toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: /Acheter/ }));
    expect(buy).toHaveBeenCalledOnce();
    expect(screen.getByRole("status")).toHaveTextContent(
      /Booster Guérisseurs Émeraude ajouté/,
    );
    await userEvent.click(
      screen.getByRole("button", { name: /Ouvrir mes boosters/ }),
    );
    expect(open).toHaveBeenCalledOnce();
  });
  it("disables purchases with insufficient currency and keeps unpaid offers explicit", async () => {
    const buy = vi.fn(() => false);
    render(
      <PortraitShop
        progress={{ ...progress, coins: 80 }}
        onBuy={buy}
        onOpen={() => {}}
        monetization={loadMonetization()}
      />,
    );
    expect(screen.getByRole("button", { name: /Acheter/ })).toBeDisabled();
    expect(screen.getByText(/Il te manque 320/)).toBeInTheDocument();
  ...
  it("shows an honest placeholder rather than a wrong family sheet when card art fails", () => {
    const card = { ...cards[0], assetPath: "/assets/cards/test.webp" };
    render(<CardArtwork card={card} />);
    fireEvent.error(screen.getByRole("img", { name: card.name }));
    expect(
      screen.getByRole("img", { name: `Illustration à venir : ${card.name}` }),
    ).toBeInTheDocument();
  });
});
