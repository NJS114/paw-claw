...
import type {PlayerCompanion} from './data/companion';

type LobbyTarget='battle'|'collection'|'deck'|'progression'|'shop'|'boosters'|'profile';
type Props={progress:Progression;ownedCount:number;missions?:MissionProgress;companion?:PlayerCompanion;onNavigate:(target:LobbyTarget)=>void};

export function MobileLobby({progress,ownedCount,missions,companion,onNavigate}:Props){
 ...
 return <section className="mobile-lobby" data-lobby-period={lobbyPeriod} aria-label="Accueil Paw & Claw">
  ...
  <header className="lobby-player-strip">
   <button className="lobby-profile" onClick={()=>onNavigate('profile')} aria-label="Ouvrir le profil">
    <span className="lobby-avatar"><GameAsset assetId={companion?.species==='Chiot'?'lobby.hero-dog':'lobby.hero-cat'} decorative loading="eager"/></span>
    <span className="lobby-player-copy"><strong>{companion?.name??'Mon compagnon'}</strong><span>Niveau {progress.level}</span><small>Personnaliser</small><progress aria-label="Expérience du joueur" value={progress.xp} max={xpTarget}/></span>
   </button>
  ...
  <aside className="lobby-rewards" aria-label="Objectifs et récompenses">
   <button className="lobby-booster-card lobby-frame" onClick={()=>onNavigate('boosters')} aria-label={`Boosters · ${progress.sealedBoosters} disponibles`}>
    <GameAsset assetId="lobby.booster" decorative/>
    <span><small>ROYAUMES &amp; LÉGENDES · {progress.sealedBoosters} disponibles</small><strong>Une nouvelle aventure</strong><em>Découvre tes boosters</em></span><span className="lobby-chevron" aria-hidden="true">›</span>
   </button>
  ...
  <footer className="lobby-footer"><span className="lobby-welcome">Bienvenue dans Paw &amp; Claw.<br/><em>Que l’aventure commence !</em></span></footer>
 </section>
}
