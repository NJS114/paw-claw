...
type LegacySavedBattle={version:2;startedAt:number;updatedAt:number;maxMomentum:number;stats?:BattleStats;state:BattleState};

function migrateLegacy(parsed:LegacySavedBattle):SavedBattle{
  return {...parsed,version:3,stats:{...emptyBattleStats(),...(parsed.stats||{})},phase:'player',checkpoint:'stable-player'};
}
...
export function loadBattle():SavedBattle|null{
  try{
    ...
    if(raw){
      ...
      if(parsed.version===3)return {...parsed,stats:{...emptyBattleStats(),...(parsed.stats||{})},phase:parsed.phase||'player',checkpoint:parsed.checkpoint||'stable-player'};
    }
    const legacyRaw=localStorage.getItem(LEGACY_ACTIVE_KEY);
    if(!legacyRaw)return null;
    const legacy=JSON.parse(legacyRaw) as LegacySavedBattle;
...
export function saveBattle(saved:SavedBattle){localStorage.setItem(ACTIVE_KEY,JSON.stringify({...saved,updatedAt:Date.now()}))}
...
export function finishBattle(saved:SavedBattle,result:'win'|'lose'){
  ...
  const entry:BattleHistoryEntry={id:`match-${saved.startedAt}`,startedAt:saved.startedAt,endedAt,durationMs:Math.max(0,endedAt-saved.startedAt),result,turns:saved.state.turn,playerHp:saved.state.player.heroHp,enemyHp:saved.state.enemy.heroHp,maxMomentum:saved.maxMomentum,stats:{...emptyBattleStats(),...(saved.stats||{})}};
