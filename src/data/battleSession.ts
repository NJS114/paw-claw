...
function normalizeState(state: BattleState): BattleState {
  return {
    ...state,
    player: {
      ...state.player,
      constructs: state.player.constructs ?? [],
      comboMarks: state.player.comboMarks ?? [],
    },
    enemy: {
      ...state.enemy,
      constructs: state.enemy.constructs ?? [],
      comboMarks: state.enemy.comboMarks ?? [],
    },
  };
}
function normalize(parsed: SavedBattle): SavedBattle {
  const phase = parsed.phase || "player",
    checkpoint =
      parsed.checkpoint || (phase === "intro" ? "intro" : "stable-player"),
    state = normalizeState(cloneState(parsed.state)),
    checkpointState = normalizeState(
      parsed.checkpointState
        ? cloneState(parsed.checkpointState)
  ...
  return {
    ...
    state,
    version: 3,
    stats: { ...emptyBattleStats(), ...(parsed.stats || {}) },
    phase,
    checkpoint,
    checkpointState,
  };
...
function recoverInterrupted(saved: SavedBattle): SavedBattle {
  ...
  if (
    (normalized.phase === "enemy" || normalized.phase === "combat") &&
    normalized.checkpointState
  )
    return {
      ...normalized,
      state: cloneState(normalized.checkpointState),
      phase: "player",
      checkpoint: "stable-player",
      updatedAt: Date.now(),
    };
  if (normalized.phase === "result")
    return { ...normalized, phase: "player", checkpoint: "stable-player" };
  return normalized;
}
...
export function saveBattle(saved: SavedBattle) {
  localStorage.setItem(
    ACTIVE_KEY,
    JSON.stringify({ ...normalize(saved), updatedAt: Date.now() }),
  );
}
...
export function finishBattle(saved: SavedBattle, result: BattleResult) {
...
export function loadBattleHistory(): BattleHistoryEntry[] {
  try {
    ...
    return rows.map((row) => ({
      ...
      durationMs:
        row.durationMs ??
        Math.max(0, (row.endedAt ?? 0) - (row.startedAt ?? 0)),
      stats: { ...emptyBattleStats(), ...(row.stats || {}) },
    }));
  } catch {
    return [];
  }
