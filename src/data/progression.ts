...
export function battleVictory(p: Progression): Progression {
  return addXp(
    {
      ...p,
      coins: p.coins + ECONOMY.winCoins,
      gems: p.gems + ECONOMY.winGems,
      wins: p.wins + 1,
    },
    60,
  );
}
export function battleDefeat(p: Progression): Progression {
  return addXp(
    { ...p, coins: p.coins + ECONOMY.lossCoins, losses: p.losses + 1 },
    25,
  );
}
export function battleDraw(p: Progression): Progression {
  return addXp(
    { ...p, coins: p.coins + ECONOMY.drawCoins, draws: p.draws + 1 },
    35,
  );
}
export function buyBooster(
  p: Progression,
  price: number = ECONOMY.standardBoosterCoins,
): Progression | null {
...
export function openBooster(p: Progression): Progression | null {
  ...
  return {
    ...p,
    sealedBoosters: p.sealedBoosters - 1,
    boostersOpened: p.boostersOpened + 1,
  };
}
