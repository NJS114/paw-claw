...
export function BattleDeckHud({playerDeck,enemyDeck,playerHand,enemyHand}:{playerDeck:number;enemyDeck:number;playerHand:number;enemyHand:number}){
 ...
 return <div className="battle-deck-hud" aria-label="État des pioches">
  <div className={`deck-zone enemy ${pulse==='enemy'?'drawing':''}`}>
   <div className="hidden-hand" aria-label={`${enemyHand} cartes dans la main adverse`}>
    {Array.from({length:Math.min(enemyHand,5)}).map((_,i)=><span key={i} className="card-back real-card-back" style={{'--i':i} as React.CSSProperties}><GameAsset assetId="booster.card-back" decorative loading="eager"/></span>)}
   </div>
   <DeckStack count={enemyDeck} label="Pioche adverse"/>
  </div>
...
function DeckStack({count,label}:{count:number;label:string}){return <div className={`deck-stack ${count===0?'empty':''}`} aria-label={`${label} : ${count} cartes`}><span/><span/><span/><div className="deck-back real-deck-back"><GameAsset assetId="booster.card-back" decorative loading="eager"/><small>{count}</small></div><em>{label}</em></div>}
