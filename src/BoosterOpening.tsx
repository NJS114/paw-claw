...
export function BoosterOpening({owned,available,onOpen,onShop}:Props){
 ...
 useEffect(()=>{
  ...
  const id=window.setTimeout(()=>setPhase(phase==='charge'?'tear':'reveal'),phase==='charge'?620:720);
 ...
 return <section className={`booster-opening-v2 phase-${phase} ${legendary?'has-legendary':''}`} aria-label="Ouverture de booster">
  <div className="booster-stage" onClick={phase==='reveal'?revealNext:undefined}>
   ...
   <div className="booster-pack-focus">
    ...
    <GameAsset assetId="booster.standard-violet" className="booster-pack-art"/>
    <div className="booster-tear-line" aria-hidden="true"/>
   </div>
   {phase==='idle'&&<div className="booster-callout"><p>ROYAUMES & LÉGENDES</p><h2>Ouvre ton booster</h2><span>{available} booster{available>1?'s':''} disponible{available>1?'s':''}</span><button className="game-cta" onClick={e=>{e.stopPropagation();start()}}>{available>0?'Ouvrir':'Aller à la boutique'}</button></div>}
   {(phase==='charge'||phase==='tear')&&<div className="booster-opening-status" aria-live="polite"><strong>{phase==='charge'?'Énergie du royaume…':'Ouverture…'}</strong><span>Prépare la révélation</span></div>}
    ...
    <div className="reveal-stack" aria-live="polite">
     {cards.map((card,index)=>{
      ...
      return <article key={`${card.id}-${index}`} className={`reveal-card rarity-${slug(card.rarity)} ${visible?'is-revealed':'is-hidden'} ${index===revealed-1?'is-latest':''}`} style={{'--offset':offset} as React.CSSProperties}>
       {visible?<><div className="reveal-card-art"><img src={card.assetPath??'/assets/generated/carte-bleue.webp'} alt={card.name}/>{newIds.has(card.id)&&<span className="reveal-new">NOUVELLE</span>}</div><div className="reveal-card-meta"><small>{card.rarity}</small><strong>{card.name}</strong>{card.type==='Héros'&&<span>ATQ {card.atk} · PV {card.hp}</span>}</div></>:<div className="card-back-v2"><span>PAW & CLAW</span><b>TOUCHER</b></div>}
      </article>
     })}
    </div>
