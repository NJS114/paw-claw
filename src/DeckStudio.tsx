...
export function DeckStudio({owned,activeDeck,onActiveDeckChange,onPlay}:{owned:OwnedCards;activeDeck:SavedDeck;onActiveDeckChange:(d:SavedDeck)=>void;onPlay:()=>void}){
 ...
 <div className="cozy-feature" aria-label="Deck sélectionné">
  ...
  <div className="cozy-deck-caption"><strong>{active.name}</strong><span>{active.cardIds.length}/{DECK_SIZE} cartes</span><button className="paw-deck-menu" aria-label="Gérer ce deck" aria-controls="deck-options" onClick={()=>{const el=optionsRef.current;if(el){el.open=true;el.scrollIntoView({block:"center"})}}}><CozyIcon name="more"/></button></div>
 </div>
 <div className="deck-carousel-controls" aria-label="Choisir un deck"><button type="button" onClick={()=>chooseDeck(-1)} disabled={orderedDecks.length<2} aria-label="Deck précédent"><CozyIcon name="chevron-left"/></button><div className="deck-carousel-dots">{orderedDecks.map((d,i)=><button key={d.id} type="button" className={d.id===active.id?'active':''} aria-label={`Afficher ${d.name}`} aria-pressed={d.id===active.id} onClick={()=>{setActiveId(d.id);setFamily('Toutes');setConfirmed(false);setPanel(d.cardIds.length?'deck':'collection')}}>{i+1}</button>)}</div><button type="button" onClick={()=>chooseDeck(1)} disabled={orderedDecks.length<2} aria-label="Deck suivant"><CozyIcon name="chevron-right"/></button></div>
 {validation.issues.length>0&&<div className="deck-issues" role="status"><p>{validation.issues[0]}</p></div>}
 <div className="cozy-vault-heading"><h3><CozyIcon name="paw"/>Mes cartes</h3><select aria-label="Filtrer par famille" value={family} onChange={e=>{setFamily(e.target.value);setPanel('collection')}}><option value="Toutes">Toutes</option>{familyOptions.map(f=><option key={f}>{f}</option>)}</select></div>
 ...
 <div className="deck-launch"><button className="primary" disabled={!validation.valid} onClick={()=>{saveDeckLibrary(lib);saveActiveDeckId(active.id);onActiveDeckChange(active);setConfirmed(true)}}><CozyIcon name={confirmed?'check':'paw'}/><span>{confirmed?'Deck validé':'Valider le deck'}</span><CozyIcon name="paw"/></button>{confirmed&&<><span className="sr-only" role="status">Deck validé et prêt à jouer.</span><button onClick={onPlay}>Combattre avec ce deck</button></>}</div>
 <details ref={optionsRef} id="deck-options" className="cozy-deck-options"><summary>Gérer mes decks</summary><div className="deck-create-actions"><button onClick={()=>make('Chat')}>Nouveau deck Chats</button><button onClick={()=>make('Chien')}>Nouveau deck Chiens</button></div><div className="deck-editor-toolbar"><label className="cozy-field-label" htmlFor="deck-name">Nom du deck</label><input id="deck-name" value={active.name} onChange={e=>save({...active,name:e.target.value.slice(0,28)})} aria-label="Nom du deck"/><span className={`deck-validity ${validation.valid?'valid':'invalid'}`}>{validation.valid?'Équipage prêt':'Deck à compléter'}</span><button onClick={()=>setLib(l=>toggleFavorite(l,active.id))}>{active.favorite?'Retirer favori':'Définir favori'}</button><button onClick={removeDeck} disabled={lib.decks.length<=1}>Supprimer</button></div></details>
