...
export function DeckStudio({owned,activeDeck,onActiveDeckChange,onPlay}:{owned:OwnedCards;activeDeck:SavedDeck;onActiveDeckChange:(d:SavedDeck)=>void;onPlay:()=>void}){
 ...
 <header className="deck-studio-head"><h2>Mes decks</h2></header>
 ...
 <div className="deck-library-strip" aria-label="Choisir un deck">{[...lib.decks].sort((a,b)=>Number(b.favorite)-Number(a.favorite)||b.updatedAt-a.updatedAt).map(d=><button key={d.id} aria-pressed={d.id===active.id} className={d.id===active.id?'active':''} onClick={()=>{setActiveId(d.id);setFamily('Toutes');setConfirmed(false);setPanel('deck')}}><strong>{d.name}</strong><span>{d.species}s · {d.cardIds.length}/{DECK_SIZE}</span>{d.favorite&&<small>Favori</small>}</button>)}</div>
