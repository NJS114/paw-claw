import {factionName, displayDeckName} from './data/factions';
...
export function DeckStudio({owned,activeDeck,onActiveDeckChange,onPlay}:{owned:OwnedCards;activeDeck:SavedDeck;onActiveDeckChange:(d:SavedDeck)=>void;onPlay:()=>void}){
 ...
 return <section className="deck-studio"><div className="deck-studio-head"><div><p className="eyebrow">DECKS · ORCHESTRATION</p><h2>Atelier des decks</h2><p>Compose ton équipe. Chaque carte compte.</p></div><div className="deck-create-actions"><button onClick={()=>make('Chat')}>Équipe Moustaches</button><button onClick={()=>make('Chien')}>Équipe Truffes</button></div></div>
 <div className="deck-library-strip">{[...lib.decks].sort((a,b)=>Number(b.favorite)-Number(a.favorite)||b.updatedAt-a.updatedAt).map(d=><button key={d.id} className={d.id===active.id?'active':''} onClick={()=>setActiveId(d.id)}><strong>{displayDeckName(d.name)}</strong><span>{factionName(d.species)} · {d.cardIds.length}/{DECK_SIZE}</span>{d.favorite&&<small>Favori</small>}</button>)}</div>
 ...
 <section className="deck-card-vault" aria-label={panel==='deck'?active.name:`Cartes ${factionName(active.species)} disponibles`}>
