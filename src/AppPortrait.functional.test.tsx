...
describe("Portrait app integration", () => {
  it("keeps the four portrait destinations reachable and saves a real booster purchase", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Galerie royale" }),
    ).toBeInTheDocument();
    expect(container.querySelector(".screen-background-art")).toHaveAttribute(
      "src",
      "/assets/backgrounds/bg-collection-gallery-2d-v9.webp",
    );
    ...
    await user.click(
      screen.getByRole("button", { name: "Acheter · 400 pièces" }),
    );
    expect(
      screen.getByRole("region", { name: "Ouverture de booster" }),
    ).toBeInTheDocument();
