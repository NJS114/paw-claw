...
function opponentPoolFor(species:Species){
 const candidates=cards.filter(c=>c.type==='Héros'&&speciesOf(c)===species);
 if(!candidates.length)return cards.filter(c=>c.type==='Héros').slice(0,20);
 const out:CardData[]=[];
 for(let i=0;i<24;i++)out.push(candidates[i%candidates.length]);
 return out;
}

export function Matchmaking({playerPool,onWin,onLose,onEditDeck}:{playerPool:CardData[];onWin:()=>void;onLose:()=>void;onEditDeck:()=>void}){
 const playerSpecies=useMemo(()=>deckSpecies(playerPool),[playerPool]);
 const rivalSpecies=opposite(playerSpecies);
 const enemyPool=useMemo(()=>opponentPoolFor(rivalSpecies),[rivalSpecies]);
 ...
 useEffect(()=>{
   ...
   searchTimer.current=window.setTimeout(()=>setPhase('prepare'),1300);
 ...
 if(phase==='battle')return <BattleArena playerPool={playerPool} enemyPool={enemyPool} playerSpecies={playerSpecies} enemySpecies={rivalSpecies} onWin={onWin} onLose={onLose}/>;

 return <section className="matchmaking-screen">
   <div className="matchmaking-backdrop"/>
   {phase==='search'?<div className="match-search-card">
     <p className="eyebrow">ARÈNE CLASSÉE</p>
     <h1>Recherche d’un adversaire</h1>
     <div className="search-orbit" aria-hidden="true"><span/><span/><span/></div>
     <p>Ton deck <strong>{playerSpecies}</strong> cherche automatiquement un adversaire <strong>{rivalSpecies}</strong>.</p>
     <button onClick={onEditDeck}>Modifier le deck</button>
   </div>:<div className="match-ready-card">
