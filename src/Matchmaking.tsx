...
const GUIDE_KEY='paw-claw.combat-guide.seen.v1';
...
function loadGuide(){try{return localStorage.getItem(GUIDE_KEY)!=='1'}catch{return true}}
export function Matchmaking({playerPool,onWin,onLose,onDraw,onEditDeck}:{playerPool:CardData[];onWin:()=>void;onLose:()=>void;onDraw:()=>void;onEditDeck:()=>void}){
 ...
 const[phase,setPhase]=useState<Phase>('search'),[seconds,setSeconds]=useState(PREP_SECONDS),[guideOpen,setGuideOpen]=useState(loadGuide);const searchTimer=useRef<number|null>(null),locked=useRef(false);
 function launch(){if(locked.current)return;locked.current=true;try{localStorage.setItem(GUIDE_KEY,'1')}catch{}clearBattle();setPhase('battle')}
 ...
 return <section className="matchmaking-screen"><div className="matchmaking-backdrop"/>{phase==='search'?<div className="match-search-card"><p className="eyebrow">ARÈNE CLASSÉE</p><h1>Recherche d’un adversaire</h1><div className="search-orbit" aria-hidden="true"><span/><span/><span/></div><p>Ton deck <strong>{playerSpecies}</strong> cherche automatiquement un adversaire <strong>{rivalSpecies}</strong>.</p><button onClick={onEditDeck}>Modifier le deck</button></div>:<div className="match-ready-card"><p className="eyebrow">ADVERSAIRE TROUVÉ</p><div className="versus-species"><div><small>TON CAMP</small><strong>{playerSpecies.toUpperCase()}S</strong><span>{playerPool.length} cartes</span></div><b>VS</b><div><small>RIVAL</small><strong>{rivalSpecies.toUpperCase()}S</strong><span>Deck adverse verrouillé</span></div></div><h2>Prépare ton entrée dans l’arène</h2><p>Le deck actif sera verrouillé au lancement. Dix secondes suffisent pour vérifier ton choix sans ralentir la partie.</p><button className="combat-guide-toggle" onClick={()=>setGuideOpen(v=>!v)} aria-expanded={guideOpen}>{guideOpen?'Masquer le guide':'Règles du combat'}</button>{guideOpen&&<div className="combat-guide" role="note"><div><strong>1. Construis le front</strong><span>Pose tes cartes dans l’une des 7 lignes. Une carte ennemie en face provoque un duel simultané.</span></div><div><strong>2. Gère ton énergie</strong><span>Chaque carte coûte de l’énergie. Les familles, le Momentum et les histoires peuvent modifier un tour.</span></div><div><strong>3. Anticipe la pioche</strong><span>Main limitée à 5 cartes. Quand la pioche est vide, la fatigue augmente à chaque nouvelle tentative de pioche.</span></div><div><strong>4. Fin de partie</strong><span>Le premier héros à 0 PV perd. Double K.O. ou limite de 40 tours : égalité.</span></div></div>}<div className="match-countdown" aria-live="polite"><span style={{'--progress':`${seconds/PREP_SECONDS*100}%`} as CSSProperties}>{seconds}</span><small>secondes</small></div><div className="match-actions"><button onClick={onEditDeck}>Changer de deck</button><button className="primary" onClick={launch}>Prêt</button></div></div>}</section>
}
