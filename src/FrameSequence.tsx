...
export type FrameAtlas = {src:string; columns:number; rows:number; durations:number[]};
...
export function FrameSequence({atlas, label, fallback}:{atlas:FrameAtlas; label:string; fallback:React.ReactNode}) {
 ...
 useEffect(() => {
  ...
  image.onload = () => {if(live)setReady(true);};
 ...
 return <span ref={root} className="frame-sequence" data-frame={frame}>
  {ready && !failed ? <span className="frame-sequence-cell" role="img" aria-label={label} style={{backgroundImage:`url("${atlas.src}")`,backgroundSize:`${atlas.columns * 100}% ${atlas.rows * 100}%`,backgroundPosition:`${x}% ${y}%`}}/> : fallback}
 </span>;
}
